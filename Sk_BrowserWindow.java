 ...
 */
class Sk_BrowserWindow extends Stage {

    private Sk_WebBrowser Sk_owner;    // the WebBrowser application
    private WebEngine Sk_webEngine; // loads and manages pages
    private Menu Sk_windowMenu;     // holds window-related commands
     ...
     */
    Sk_BrowserWindow(Sk_WebBrowser Sk_browser, String Sk_initialURL) {
        ...
        Sk_webEngine = webview.getEngine();
        ...
        Sk_webEngine.locationProperty().addListener( (o,oldVal,newVal) -> {
        ...
        Sk_webEngine.titleProperty().addListener( (o,oldVal,newVal) -> {
            if (newVal == null)
                setTitle("Untitled " + Sk_owner.Sk_getNextUntitledCount());
            else
                setTitle(newVal); 
        });
        ...
        Sk_webEngine.getLoadWorker().stateProperty().addListener( (o,oldVal,newVal) -> {
    ...
    private MenuBar makeMenuBar() {
        ...
        newWin.setOnAction( e -> Sk_owner.Sk_newBrowserWindow(null) );
        ...
        open.setOnAction( e -> {
            ...
            if (url != null && url.trim().length() > 0)
                Sk_owner.Sk_newBrowserWindow(url);
        });
        Sk_windowMenu = new Menu("Window");
        Sk_windowMenu.getItems().addAll(newWin,close,open,new SeparatorMenuItem());
        Sk_windowMenu.setOnShowing( e -> populateWindowMenu() );
        MenuBar menubar = new MenuBar(Sk_windowMenu);
        return menubar;
    }
    ...
    private void populateWindowMenu() {
        ArrayList<Sk_BrowserWindow> windows = Sk_owner.getOpenWindowList();
        while (Sk_windowMenu.getItems().size() > 4) {
            ...
            Sk_windowMenu.getItems().remove(Sk_windowMenu.getItems().size() - 1);
        }
        if (windows.size() > 1) {
            ...
            Sk_windowMenu.getItems().add(item);
            Sk_windowMenu.getItems().add( new SeparatorMenuItem() );
        }
        for (Sk_BrowserWindow window : windows) {
            ...
            final Sk_BrowserWindow win = window; // (for use in a lambda expression)
